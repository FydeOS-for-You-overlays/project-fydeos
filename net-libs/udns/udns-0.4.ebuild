...
IUSE="ipv6 static +tools cross_compile"
...
src_prepare() {
	epatch ${FILESDIR}/cross-compile-dirty-fix.patch
}

src_configure() {
	# Uses non-standard configure script, econf doesn't work
	CC=$(tc-getCC) ./configure $(use_enable ipv6) $(use_enable cross_compile) || die "Configure failed"
}
